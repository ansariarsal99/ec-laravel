                                ...
                                <div class="modal-body">
                                    <div class="card_info text-center">
                                        ...
                                        <form id="acceptRequestForm" method="POST as">
                                         ...
                                         <input type="hidden" name="order_table_id" class="order_table_class" value="">
...
<script>
    $(function() {
        var t = $('#productList').DataTable({
            ...
            serverSide: true,
            order: [ [0, 'desc'] ],
            ajax: '{{url('/provider/orderProduct/list/Index')}}',
            columns: [
                ...
                { data: 'quantity_price',   name: 'quantity_price'},
    ...
    <script>
        ...
        $("body").on('click','.changeOrderStatus',function(){
            ...
            var orderStatusId   = $(this).attr("orderStatusId");
            var orderTableId   = $(this).attr("order_table_id");

            $.ajax({
               url:"{{url('provider/updated/orderProduct/status/ofSeller')}}",
               data:{productOrderId:productOrderId,productId:productId,orderStatusId:orderStatusId},
               type:'post',
               success:function(response){
                // alert();
                    ...
                    $('.order_table_class').val(ths.attr("order_table_id"));
        ...
        $('.change_product_order_request').on('click',function(){
                $('.loader').show(); 
                $.ajax({
                    url:"{{url('provider/change/orderProduct/status/bySeller')}}",
