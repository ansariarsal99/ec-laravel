...
<section class="outer_db_wraper db_seller_items_list">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <div class="sh_pric_tble quantity_example">
                                                                ...
                                                                <div class="row m-0 mb-3">
                                                                    <input type="hidden" class="weight-count" value="{{count($product->productWeight)}}">
                                                                    <div class="col-sm-3 inr_datat p-0 product-weight">
                                                                        @foreach($product->productWeight as $weight)
                                                                        <p class="quantity-class-{{$weight->id}}">{{@$weight->quantity}}</p>
                                                                        @endforeach
                                                                    </div>
                                                                    <div class="col-sm-3 inr_datat p-0 product-weight">
                                                                        @foreach($product->productWeight as $weight)
                                                                        <p class="pieces-class-{{$weight->id}}">{{@$weight->pcs}}</p>
                                                                        @endforeach
                                                                    </div>
                                                                    <div class="col-sm-3 inr_datat p-0 product-weight">
                                                                        @foreach($product->productWeight as $weight)
                                                                        <p class="price-class-{{$weight->id}}">{{@$weight->price}}</p>
                                                                        @endforeach
                                                                    </div>
                                                                    <div class="col-sm-3 inr_datat p-0 product-weight">
                                                                        @foreach($product->productWeight as $weight)
                                                                        <p class="action-class-{{$weight->id}}">
                                                                            <a href="javascript:;" class="edit-weight text-info" weight_id="{{$weight->id}}">Edit</a>
                                                                            <a href="javascript:;" class="delete-btn delete-weight text-danger" del_id="{{$weight->id}}">Delete</a>
                                                                        </p>
                                                                        @endforeach
                                                                    </div>
                                                                </div>
                                                            ...
                                                            <div class="col-6 col-xs-12">
                                                                <div class="form-group">
                                                                    ...
                                                                    <span class="code">
                                                                        ...
                                                                        <img src="data:image/png;base64,{{DNS1D::getBarcodePNG($product->bar_code, 'C93')}}" alt="barcode" style="width: 254px; height: 30px"  />
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        </div>
    ...
    <div class="modal" id="edit_weight">
        <div class="modal-dialog">
            <div class="modal-content">

                <!-- Modal Header -->
                <div class="modal-header">
                    <h4 class="modal-title">Edit Weight & Quantity</h4>
                ...
                <div class="modal-body">
                    <div class="add_form">
                        <form id="edit-weight-form" method="post" action="">
                            <div class="form-group">
                                <label>Select Pcs.</label>
                                <input type="text" name="pcs" class="form-control pcs-class" value="">
                            </div>
                            <div class="form-group">
                                <label>Select Weight</label>
                                <input type="text" name="quantity" class="form-control weight-class" value="">
                            </div>
                            <input type="hidden" name="price" class="product_price" value="">
                            <input type="hidden" name="wightId" class="weight-id-class" value="">
                        </form>
                    </div>
                </div>
                <!-- Modal footer -->
                <div class="modal-footer">
                    <button type="button" class="btn btn_theme edit-weight-btn"><span>Save</span></button>
                </div>
            </div>
      </div>
    </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '.edit-weight-btn', function(event){
        event.preventDefault();
        price = $('.price_per_unit').val();
        $('.product_price').val(price);
        // $('#edit-weight-form').submit();
        if($("#edit-weight-form").valid()){
           // test for validity
            $.ajax({
            url:"{{url('provider/products/edit/weight')}}",
            type: "post",
            data:$('#edit-weight-form').serialize(),
            success:function(data){
                // alert($('.quantity-class-'+data.weight.id).text());
                $('.pieces-class-'+data.weight.id).text(data.weight.pcs);
                $('.quantity-class-'+data.weight.id).text(data.weight.quantity);
                $('.price-class-'+data.weight.id).text(data.weight.price);
...
<script type="text/javascript">
    $(document).on('click', '.edit-weight', function(){
        ...
        $.ajax({
            url:"{{url('provider/products/weight')}}",
            type:"post",
            data:{weightId:weightId},
            success:function(data){
                $('.pcs-class').val(data.weight.pcs);
                $('.weight-class').val(data.weight.quantity);
                $('.weight-id-class').val(data.weight.id);
                if(!price) {
