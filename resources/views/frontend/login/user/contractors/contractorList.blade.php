    ...
    <div class="pagntn">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Contractor's List</li>
            </ol>
        </nav>
    </div>
    <section class="cntrctr_lst_page_sec">
        <div class="custom_container">
            <div class="wrp_cntrctr_lst">
                <div class="row">
                    <div class="col-sm-3">
                        <div class="filtr_left cntrctr_fltrs">
                            <fieldset>
                                <form id="providerFltrsForm" action="{{url('/contractor/list')}}" method="GET">
                                    <h5>Narrow Result</h5>
                                    ...
                                    <div class="botom_meta_dsgnr filtr_btn text-center my-3">
                                        <a class="btn btn_theme providerFltrsCls" href="{{url('/contractor/list')}}"><span>Reset</span></a>
                    ...
                    <div class="col-sm-9">
                        <div class="cntrctr_main_rgt">
                            <div class="fltr_topp pos_rel">
                                <div class="section-heading">
                                    <h2>Contractor's List</h2>
                                </div>
                                @if($contractors->total()>0)
                                    <a class="btn btn_theme out_btn_head"><span>Request Proposal of <span class="quot_for_count">{{ $selectedCount>0 ? $selectedCount : '' }}</span> Contractors</span></a>
                                @endif
                            </div>
                            ...
                            @if($contractors->total()>0)
                                <div class="wrap_cntrctr_main">
                                            ...
                                            <div class="single_list_desgnr d-flex">
                                                ...
                                                <div class="dsgnr_dtl">
                                                    <div class="top_meta_dsgnr d-flex justify-content-between">
                                                        @if($contractor['userPropertyDetail']['type']=='company')
                                                            <a href="{{url('/contractor/detail/'.base64_encode($contractor['id']))}}">{{ucfirst(@$contractor['company_name'])}}</a>
                                                        @else
                                                            <a href="{{url('/contractor/detail/'.base64_encode($contractor['id']))}}">{{ucfirst(@$contractor['contact_name'])}} {{ucfirst(@$contractor['contact_last_name'])}}</a>
                                                        @endif
                                                        @if(Auth::check() && (Auth::user()->user_type_id==1 || Auth::user()->user_type_id==2))
                                                            <div class="custom-control custom-checkbox">
                                                                <input type="checkbox" @if(in_array($contractor['id'],$selectedProviderIds)) checked="" @endif value="{{@$contractor['id']}}" data-id="{{@$contractor['id']}}" class="custom-control-input cstm_chek" id="customCh{{$key}}" name="provider_ids[]">
                                                    ...
                                                    <div class="middle_meta_dsgnr d-flex justify-content-between align-items-center">
                                                        <div class="left_info_dsgnr">
                                                            ...
                                                            <!-- <p><strong>Location:</strong> <a class="cp_loca"> {{@$contractor['location']}}</a></p> -->
                                                            <p><strong>Experience:</strong> +{{@$contractor['experience']}} (In @if(@$contractor['experience']<=1) Year @else Years @endif)</p>
                                                            <p><strong>Type of Entity:</strong> {{@$contractor['userPropertyDetail']['name']}}</p>
                                                            <p><strong>Category:</strong> {{@$contractor['userPropertyDetail']['name']}}</p>
                                                            <p><strong>City:</strong> {{@$contractor['userDefaultStoreLocation']['cityDetail']['name']}}</p>
                                                            <p><strong>Type of Services :</strong> @foreach($contractor['userSelectedServicesDetail'] as $key => $serviceDetail) @if($key>=1) ,@endif{{$serviceDetail['userServiceDetail']['name']}} @endforeach </p>
                                                            <p><strong>Project Fields :</strong> @foreach($contractor['userSelectedProjectFieldsDetail'] as $key => $projectFieldDetail) @if($key>=1) ,@endif{{$projectFieldDetail['userProjectFieldDetail']['name']}} @endforeach </p>
                                                        </div>
                                                        <div class="rgt_ratng">
                                                    ...
                                                    <div class="botom_meta_dsgnr text-right">
                                                        ...
                                                        <a class="btn btn_theme" href="{{url('/contractor/detail/'.base64_encode($contractor['id']))}}"><span>Read More</span></a>
                                                    </div>
                                                </div>
                                            </div>
                                        @endif
                                    ...
                                    <!-- <div class="single_list_desgnr d-flex">
                                        ...
                                        <div class="dsgnr_dtl">
                                            <div class="top_meta_dsgnr d-flex justify-content-between">
                                                <a href="{{url('/contractor/detail')}}">William Petrson</a>
                                                <div class="custom-control custom-checkbox">
                                            ...
                                            <div class="middle_meta_dsgnr d-flex justify-content-between align-items-center">
                                                <div class="left_info_dsgnr">
                                                    ...
                                                    <p><strong>Service Type:</strong> Interior</p>
                                                </div>
                                                <div class="rgt_ratng">
                                                    <span><small>9.8</small>/10 <br>140 Customer Reviews</span>
                                                </div>
                                            ...
                                            <div class="botom_meta_dsgnr text-right">
                                                <a class="btn btn_theme" href="{{url('/contractor/detail')}}"><span>Read More</span></a>
                                            </div>
                                        </div>
                                    </div> -->
                                </div>
                                <div class="numbr_pagintn mt-3">
                                    {{ $contractors->appends(request()->except('page'))->links() }}
                                </div>
                            @else
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("body").on('click','#customChec1',function(){
            ...
            if ($(this).is(":checked")) {
                $(this).closest('.cntrctr_main_rgt').find('.cstm_chek').prop('checked',true);
                isChecked = 'yes';
            }else{
                $(this).closest('.cntrctr_main_rgt').find('.cstm_chek').prop('checked',false);
                isChecked = 'no';
            }
            // $(".quot_for_count").text($('.cstm_chek:checked').length);

            $(this).closest('.cntrctr_main_rgt').find('.cstm_chek').each(function(){
                thsId = $(this).data('id');
        ...
        $("body").on('click','.out_btn_head',function(){
            if (auth) {
                ...
                //     swal('Sorry, Maximum 5 contractors can be selected');           
                // }
            }else{
