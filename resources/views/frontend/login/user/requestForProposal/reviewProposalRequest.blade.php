    ...
    <section class="rev_prop_page_sec">
        <div class="custom_container">
                            ...
                            <div class="col-sm-10 offset-sm-1">
                                ...
                                <div class="fltr_topp">
                                    <div class="page_heading text-center">
                                        ...
                                        @endif
                                        <h1 class="text-uppercase">Request For Proposal</h1>
                                    </div>
                                </div>
                                ...
                                <div class="from_meta mt-5">
                                    ...
                                    <p><strong>Membership ID:</strong> {{@$reqForProposalAssignToUser['request_for_proposal_detail']['user_detail']['supplier_code']}}</p>
                                </div>
                                <div class="to_meta mt-5">
                                    @if(@$reqForProposalAssignToUser['user_detail']['user_property_detail']['type']=='company')
                                        <p><span class="mr-2"><strong>To:</strong> {{ucfirst(@$reqForProposalAssignToUser['user_detail']['company_name'])}}</span></p>
                                    @else
                                        <p><span class="mr-2"><strong>To:</strong> {{ucfirst(@$reqForProposalAssignToUser['user_detail']['contact_name'])}} {{ucfirst(@$reqForProposalAssignToUser['user_detail']['contact_last_name'])}}</span></p>
                                    @endif
                                    <p><strong>Attn:</strong> {{ucfirst(@$reqForProposalAssignToUser['user_detail']['contact_name'])}} {{ucfirst(@$reqForProposalAssignToUser['user_detail']['contact_last_name'])}}</p>
                                    ...
                                    <p><strong>Membership ID:</strong> {{@$reqForProposalAssignToUser['user_detail']['supplier_code']}}</p>
                                </div>
                                ...
                                <div class="descr_div mt-4">
                                    <h6 class="text-uppercase">Remarks/ Terms & Conditions</h6>
                                    <p>{{@$reqForProposalAssignToUser['request_for_proposal_detail']['request_remarks']}}</p>
                                </div>
            ...
            <div class="btn_sav_next d-flex justify-content-between">
                <span>
                    <button class="btn btn_theme edt_req_for_prop"><span>Edit</span></button>
                ...
                <a class="btn btn_theme" href="{{url('/user/requestForProposal/submit/'.@$encReqForProposalId)}}"><span>Submit</span></a>
            </div>
            <form class="editRequestForProposalForm" action="{{url('/user/requestForProposal')}}" method="POST">
                <input type="hidden" name="request_for_proposal_id" value="{{$encReqForProposalId}}">
            </form>
        </div>
    </section>
...
<script type="text/javascript">
    ...
    $("body").on('click','.edt_req_for_prop',function(){
        if (auth) {
            $(".editRequestForProposalForm").submit();   
        }else{
