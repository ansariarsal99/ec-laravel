...
<style type="text/css">
    ...
    .wrap_stck_inr .select2-container{
        width: 100% !important;
    }
    .wrap_stck_inr .select2-container--default .select2-selection--multiple {
        border-radius: 0;
        height: 45px;
    }
    .wrap_stck_inr .select2-container .select2-search--inline .select2-search__field {
        margin-top: 11px;
    }
    .wrap_stck_inr .select2-container--default .select2-selection--multiple .select2-selection__choice {
        margin-top: 10px;
    }
    .numbr_pagintn ul{
    ...
    <div class="pagntn">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page">Designer's List</li>
            </ol>
        </nav>
    </div>
    <section class="dsgnr_lst_page_sec">
                ...
                <div class="row">
                    <div class="col-sm-3">
                        <div class="filtr_left designr_fltrs">
                            <fieldset>
                                <form id="providerFltrsForm" action="{{url('/designer/list')}}" method="GET">
                                    <h5>Narrow Result</h5>
                                    ...
                                    <div class="botom_meta_dsgnr filtr_btn text-center my-3">
                                        <a class="btn btn_theme providerFltrsCls" href="{{url('/designer/list')}}"><span>Reset</span></a>
                        ...
                        <div class="prods_main_rgt">
                            <div class="fltr_topp">
                                <div class="section-heading">
                                    <h2>Designer's List</h2>
                                </div>
                                @if($designers->total()>0)
                                    <a class="btn btn_theme out_btn_head"><span>Request Proposal of <span class="quot_for_count">{{ $selectedCount>0 ? $selectedCount : '' }}</span> Designers</span></a>
                                @endif
                            </div>
                                    ...
                                    <div class="seldes_chk">
                                        @if(Auth::check() && (Auth::user()->user_type_id==1 || Auth::user()->user_type_id==2))
                                            <div class="custom-control custom-checkbox">
                                                ...
                                                <label class="custom-control-label" for="customChec1">Select/Deselect All</label>
                                            </div>
                                        @endif
                                    </div>
                                    <div class="dropdown">
                            ...
                            @if($designers->total()>0)
                                                ...
                                                <div class="single_list_desgnr d-flex">
                                                    <div class="cert_div d-flex flex-column">
                                                        <div class="dsgnr_img">
                                                            ...
                                                            @if(!empty($designer->profile_image) && file_exists(public_path('frontend/imgs/userProfile/'.$designer->profile_image)))
                                                                <img alt="" src="{{asset('public/frontend/imgs/userProfile/'.$designer->profile_image)}}" class="img-responsive">
                                                            @else
                                                    ...
                                                    <div class="dsgnr_dtl">
                                                        <div class="top_meta_dsgnr d-flex justify-content-between">
                                                            @if($designer['userPropertyDetail']['type']=='company')
                                                                <a href="{{url('/designer/detail/'.base64_encode($designer['id']))}}">{{ucfirst(@$designer['company_name'])}}</a>
                                                            @else
                                                                <a href="{{url('/designer/detail/'.base64_encode($designer['id']))}}">{{ucfirst(@$designer['contact_name'])}} {{ucfirst(@$designer['contact_last_name'])}}</a>
                                                            @endif
                                                            @if(Auth::check() && (Auth::user()->user_type_id==1 || Auth::user()->user_type_id==2))
                                                                <div class="custom-control custom-checkbox">
                                                                    <input type="checkbox" @if(in_array($designer['id'],$selectedProviderIds)) checked="" @endif value="{{@$designer['id']}}" data-id="{{@$designer['id']}}" class="custom-control-input cstm_chek" id="customCh{{$key}}" name="provider_ids[]">
                                                        ...
                                                        <div class="middle_meta_dsgnr d-flex justify-content-between align-items-center">
                                                            <div class="left_info_dsgnr">
                                                                ...
                                                                <!-- <p><strong>Location:</strong> <a class="cp_loca"> {{@$designer['location']}}</a></p> -->
                                                                <p><strong>Experience:</strong> +{{@$designer['experience']}} (In @if(@$designer['experience']<=1) Year @else Years @endif)</p>
                                                                <p><strong>Type of Entity:</strong> {{@$designer['userPropertyDetail']['name']}}</p>
                                                                <p><strong>Category:</strong> {{@$designer['userTypeDetail']['name']}}</p>
                                                                <p><strong>City:</strong> {{@$designer['userDefaultStoreLocation']['cityDetail']['name']}}</p>
                                                                <p><strong>Type of Services :</strong> @foreach($designer['userSelectedServicesDetail'] as $key => $serviceDetail) @if($key>=1) ,@endif{{$serviceDetail['userServiceDetail']['name']}} @endforeach </p>
                                                                <p><strong>Project Fields :</strong> @foreach($designer['userSelectedProjectFieldsDetail'] as $key => $projectFieldDetail) @if($key>=1) ,@endif{{$projectFieldDetail['userProjectFieldDetail']['name']}} @endforeach </p>
                                                            </div>
                                                            <div class="rgt_ratng">
                                                        ...
                                                        <div class="botom_meta_dsgnr text-right">
                                                            ...
                                                            <a class="btn btn_theme" href="{{url('/designer/detail/'.base64_encode($designer['id']))}}"><span>Read More</span></a>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                ...
                                <div class="numbr_pagintn mt-3">
                                    {{ $designers->appends(request()->except('page'))->links() }}
                                </div>
                            @else
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("body").on('click','#customChec1',function(){
            ...
            if ($(this).is(":checked")) {
                $(this).closest('.prods_main_rgt').find('.cstm_chek').prop('checked',true);
                isChecked = 'yes';
            }else{
                $(this).closest('.prods_main_rgt').find('.cstm_chek').prop('checked',false);
                isChecked = 'no';
            }
            // $(".quot_for_count").text($('.cstm_chek:checked').length);

            $(this).closest('.prods_main_rgt').find('.cstm_chek').each(function(){
                thsId = $(this).data('id');
        ...
        $("body").on('click','.out_btn_head',function(){
            if (auth) {
                ...
                //     swal('Sorry, Maximum 5 designers can be selected');           
                // }
            }else{
