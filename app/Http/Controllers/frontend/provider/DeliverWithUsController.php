...
class DeliverWithUsController extends Controller
{
    ...
    public function deliverWithUs(Request $request) {
        ...
        try{
            ...
            // dd(Auth::user()->id);
            $page = 'deliverWithUs';
            return view('frontend.login.provider.deliverWithUs.deliverWithUs',compact('page','countries'));
        }catch(Exception $e){
    ...
    public function deliveryPersonList(Request $request) {
        ...
        try{
            ...
            $deliveryPersons = User::with('cityDetail')
            ...
            // dd($deliveryPersons);
            $page = 'deliverWithUs';
            return view('frontend.login.provider.deliverWithUs.deliveryPersonList',compact('page','deliveryPersons'));
        }catch(Exception $e){
            \Log::error($e->getMessage());
            Session::flash('error',trans('messages.frontend.common_error'));
            return redirect()->back();
    ...
    public function deliveryPersonDetail($encDeliveryPersonId, Request $request) {

        try{
            $data = $request->all();   

            $decDeliveryPersonId = base64_decode($encDeliveryPersonId); 
            $deliveryPerson = User::with('countryDetail','cityDetail','userVehicles')->where('id',$decDeliveryPersonId)->first();
            $deliveryPerson = !empty($deliveryPerson) ? $deliveryPerson->toArray() : [];   
            // dd($deliveryPerson);
            $page = 'deliverWithUs';
            return view('frontend.login.provider.deliverWithUs.deliveryPersonDetail',compact('page','deliveryPerson'));
        }catch(Exception $e){
            \Log::error($e->getMessage());
            Session::flash('error',trans('messages.frontend.common_error'));
            return redirect()->back();
