...
class ChatController extends Controller
{
    ...
    public function messages(Request $request) {
        try{
            ...
            // dd($toUser);
            $page = 'messages';
            return view('frontend.login.user.messages.messages',compact('page','chatRooms','adminChatRoom','toUser'));
        }catch(Exception $e){
    ...
    public function userChatGetView(Request $request) {
        ...
        if ($data['chatRoomType']=='admin') {
            ...
            $chatRoom = ChatRoom::with(['senderDetail'=>function($q){ $q->select('id','user_type_id','institution_name','first_name','last_name','contact_name','contact_last_name','profile_image','is_login'); },'receiverDetail'=>function($q){ $q->select('id','user_type_id','institution_name','first_name','last_name','contact_name','contact_last_name','profile_image','is_login'); }])->where('id',base64_decode($data['chatRoomId']))->first();
            $chatRoom = !empty($chatRoom) ? $chatRoom->toArray() : [];            
        }
        // dd($chatRoom);
        ...
        if (!empty($chatRoom)) {
            $view = view('frontend.login.user.messages.chatViewCommon',compact('chatRoom','senderId'))->render();
            // return $view;
    ...
    public function userChatUpdateSidebarLastMessage(Request $request) {

        $data = $request->all();
        // dd($data);
        $updateLastMsg = ChatRoom::where('room_id',$data['room_id'])->update(['last_message'=>$data['message']]);
    }

    public function userChatOnlineStatus(Request $request) {

        if (Auth::check()) {
            $adminId = Admin::where('type','admin')->where('status','active')->value('id');
            $adminChatRoom = ChatRoom::with(['adminDetail'=>function($q){ $q->select('id','first_name','last_name','image','is_login'); }])
                                ->where('user_id',Auth::user()->id)
                                ->where('admin_id',$adminId)
                                ->first();
            $adminChatRoom = !empty($adminChatRoom) ? $adminChatRoom->toArray() : [];
            ...
            // dd($chatRooms);
            $userId = Auth::user()->id;
            return array('status'=>'success','adminChatRoom'=>$adminChatRoom,'chatRooms'=>$chatRooms,'userId'=>$userId);
        }else{
